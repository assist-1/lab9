...
void GetToNewArray(type1* arr, int size) //реализация шаблонной функции
{
...
int main()
{
  ...
  GetToNewArray<char>(arrch, sizeof(arrch)/sizeof(arrch[0]) );
  GetToNewArray<int>( arrint, sizeof(arrint)/sizeof(arrint[0]) );
  GetToNewArray<float>(arrfl, sizeof(arrfl)/sizeof(arrfl[0]) );
  GetToNewArray<double>(arrd, sizeof(arrfl)/sizeof(arrfl[0]) ); //sizeof(arr)/sizeof(arr[0]) - размер массива
}
